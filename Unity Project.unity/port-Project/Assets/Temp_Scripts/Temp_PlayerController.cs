...
public class Temp_PlayerController : MonoBehaviour ,IDamage
{
    ...
    void Update()
    {
        ...
        if (Input.GetButton("Fire1") && isShooting == false && !TempgameManager.instance.isPaused)
        {
        ...
        if (Input.GetButton("Reload") && isReloading == false && !TempgameManager.instance.isPaused)
        {
            StartCoroutine(reload());
        }
    }
    ...
    public void takeDamage(int amount)
    {
        ...
        if (HP <= 0)
        {
            TempgameManager.instance.youLose();
        }
    }
